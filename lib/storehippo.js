function StorehippoSDK(config) {
    ...
    if (config.env == 'dev') {
        self.base_url = 'http://' + config.storename + '.devhippo.com';
    }
    if (config.env == 'beta') {
        self.base_url = 'http://' + config.storename + '.betahippo.com';
    }
    if (config.version) {
    ...
    var shUrlCall = function (options, cb, err) {
        //console.log("shUrlCall called :", options, cb, err);
        var Q = require('q');
